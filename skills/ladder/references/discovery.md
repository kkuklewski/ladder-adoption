...
| Checkout drift | `behind_default_branch`, `claude_dir_behind`, `fetch_age_days` | **`claude_dir_behind: true` means the rules you are scoring are not the rules the team merged.** A guardrail exists only in checkouts that contain it, so this is scored where it matters — see 2.C2. `fetch_age_days` says how stale the comparison itself is: the probe never fetches, so a large age makes the count a lower bound, not a fact. `-1` = never fetched |
