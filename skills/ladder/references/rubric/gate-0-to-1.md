...
|---|---|---|---|
| 1.1 | Claude Code is installed and current | probe | `machine.claude_version` non-empty |
| 1.2 | A supported model is approved for use | probe | `machine.global.settings_json` or session runs with a current model (`claude-*-5*` or newer) |
