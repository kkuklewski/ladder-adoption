...
in it is `verified`, or `not_applicable` where the row allows it (state the reason). Unknown
self-reports and `assumed` findings never pass a group.
...
| 2.B3 | unit tests exist and a command runs them | probe | a test command exists (`repo.scripts.test`, Makefile `test`, pytest/cargo/go test) **and** tests exist (a config in `repo.test_configs` or test files such as `*.test.*`, `*_test.*`, `tests/`) | yes |
| 2.B4 | build command exists | probe | `repo.scripts` has `build` | yes |
| 2.B5 | e2e tests exist against a real dev environment | probe | `playwright`/`cypress` (or equivalent) in `repo.test_configs` **and** a script runs them. `not_applicable` only when the repo has no runtime surface to drive (a library or CLI with no server, UI or deployed service); say which | yes |
...
| 2.C2 | Dangerous commands are denied explicitly | inspect | deny rules cover the repo's own deploy and migrate commands (see `repo.deploy_files`, `repo.migration_dirs`) and force-push. **`repo.claude_dir_behind: true` caps this at `assumed`**, whatever the rules say: the scored checkout is behind the default branch on `.claude/`, so rules that exist upstream are absent where the work happens, and rules present here may be gone upstream. Name it in the report with `behind_default_branch` and `fetch_age_days`. Name any gap in default-branch protection separately: a lone `Bash(git push origin main:*)` pattern does not stop `git push` from main, and only branch protection, rulesets or cloud sessions (which push only their own branch) really protect it. That gap is reported, it does not decide this check | yes |
...
| 2.D2 | Security review runs automatically | probe | `repo.ci.security_scan`. A CodeQL job that runs but cannot upload results on a private repo without GitHub Code Security still counts as running; note it | yes |
| 2.D3 | Human review still gates merge (same bar as human code) | inspect | `repo.review_policy_files` (CONTRIBUTING, PR template, CODEOWNERS) or documented branch protection says agent PRs get the same human review. Nothing on disk → `unknown` | yes |
